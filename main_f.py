...
def train_contrastive_pipeline(dataset, args):
    ...
    for fold in range(0, 5):
        ...
        if args.mode in ['radiomic1D', "radio_1D"]:
        ...
        elif args.mode in ['radiomic2.5D', "radio_2.5D"]:
            #TODO
            pass
        elif args.mode in ['radiomic3D', "radio_3D"]:
            # nothing to compute from a tabular radiomics_features matrix
            print("Using 3D MRI for contrastive learning (radio_type=3D)")

            # sample_case = val_dataset.slide_data['case_id'].iloc[0]
            # sample = val_dataset.__getitem__(0)  # first sample
            # print(sample_case)

            # # The first element returned is (mri_tensors, ..., label, etc.)
            # mri_tensor = sample[0]  # shape: [1, C, D, H, W] or [C, D, H, W]

            # # Handle different tensor formats
            # if mri_tensor.ndim == 5:
            #     args.radio_input_dim = mri_tensor.shape[1]  # batch x channels x D x H x W
            # elif mri_tensor.ndim == 4:
            #     args.radio_input_dim = mri_tensor.shape[0]  # channels x D x H x W
            # else:
            #     raise ValueError(f"Unexpected MRI tensor shape: {mri_tensor.shape}")

            # print(f"Detected MRI input dim (channels): {args.radio_input_dim}")
            args.radio_input_dim = 1  # assuming single-channel MRI input
            # SNN(small) outputs 512-d features; align projection head input dim accordingly
            config = {
                "omic_input_dim": args.omic_input_dim,
                "genomics_output_dim": 512,
                "radio_input_dim": args.radio_input_dim,
                "radiomics_output_dim": 256,
                # For 3D we let the encoder infer channel count from a sample
                "projection_dim": args.projection_dim,
                "temperature": args.temperature,
                "radio_type": "3D",
            }
...
def create_parser():
    ...
    parser.add_argument('--wandb_project', type=str, default='MMContraFuse', help='W&B project name')
