...
class RadiomicMMF(nn.Module):
    def __init__(self,omic_input_dim, in_channels=4, n_classes=4, fusion='bilinear',scale_dim1=8, scale_dim2=8, gate_path=1, gate_omic=1, skip=True, model_size_omic='small', layer_num=32 ):
        self.size_dict_omic = {'small': [512,512], 'big': [1024, 1024, 1024, 256]}
        size = self.size_dict_omic[model_size_omic]
        self.fusion = fusion
        self.n_classes = n_classes

        # Constructing Radio Network
        ...
        self.mm = BilinearFusion(dim1=64, dim2=512, scale_dim1=scale_dim1, gate1=gate_path, scale_dim2=scale_dim2, gate2=gate_omic, skip=skip, mmhid=256)


        self.classifier_mm = nn.Linear(256, n_classes)

    def forward(self,x_mri=None, x_path=None, x_omic=None):
        x = self.init_layer(x_mri) # [B, 64, H/4, W/4]
        x = self.block1(x) # Residual block processes features.
        x = self.attention1(x) # attention layer highlights important spatial regions and channels. #Output shape: [B, 64, H/4, W/4]
        x = self.relu(x)
        x = self.gap(x)
        x = x.view(x.size(0), -1)
        h_mri = x
