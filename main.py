...
def main(args):
    ...
    torch.save(best_model.state_dict(), os.path.join(args.results_dir, f"best_model_{args.mode}.pt"))
    pickle_obj(val_loaders[best_fold], os.path.join(args.results_dir,f"val_loader_{args.mode}.pkl"))
    pickle_obj(val_split[best_fold], os.path.join(args.results_dir,f"val_split_{args.mode}.pkl"))
    pickle_obj(train_loaders[best_fold], os.path.join(args.results_dir,f"train_loader_{args.mode}.pkl"))
    pickle_obj(train_split[best_fold], os.path.join(args.results_dir,f"train_split_{args.mode}.pkl"))
    pickle_obj(args, os.path.join(args.results_dir,f"args_{args.mode}.pkl"))
...
parser.add_argument('--mode', type=str,
                    choices=['genomic', 'path', 'radiomic', 'radio_1D', 'radio_2.5D', 'radio_3D',
